...
int ehPrimo(int n) {
    if (n <= 1) return 0;   // Se número for 1 ou menor, não é primo
    for (int i = 2; i * i <= n; i++)   // Loop para verificar divisores
        if (n % i == 0)   // % calcula o resto da divisão
            return 0;   // Se encontrar divisor, não é primo
...
// Altere mostrarDicas para retornar o número de linhas impressas
...
void jogarFase(Jogo *jogo) {
    ...
    // Mostra o número da fase e o intervalo correspondente
    screenGotoxy(28, 2);
    setTextoBranco();
    switch (jogo->fase) {
    ...
    // Calcula a linha inicial das tentativas (4 + linhasDicas + 2 para espaçamento visual)
...
void executarJogo() {
    ...
    // Fase 5 (0-300, 5 tentativas + pergunta)
